'''
Evaluate the model prediction model for a data set using the Mean Absolute Error method.

In statistics, mean absolute error (MAE) is a measure of errors between paired observations expressing the same phenomenon. Examples of Y versus X include comparisons of predicted versus observed, subsequent time versus initial time, and one technique of measurement versus an alternative technique of measurement.

Create a function my_model_evaluation_journey_mean_absolute_error which will calculate the Mean Absolute Error for a predicted model.

You will receive two arguments: true data and predicted data. Both of them are strings, you should maybe convert them and eliminate unecessary features ;-)
...
#     Make the DFs the same number of rows
    ...
    print("52df_true: ", df_true)
    print("53df_pred: ", df_pred)
    print("54min_len: ", min_len)

#     Calculate the Mean Absolute Error
    mae = mean_absolute_error(df_true, df_pred) # Calculate the Mean Absolute Error
    print("mae: ", mae)
    return True 
...
my_model_evaluation_journey_mean_absolute_error(true_data, pred_data)
